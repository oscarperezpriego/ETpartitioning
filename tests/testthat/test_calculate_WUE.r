#require(testthat)
context("calculate_WUE_o")

test_that("calculate_WUE_o",{
  ...
  #                                                      6L), class = "data.frame")
			ans <- calculate_WUE_o(EddySample, "GPP_NT_VUT_MEAN", "VPD_F", "TA_F", C = 1.1, Z = 0.2)
			expect_true(is.vector(ans))
			expect_true(is.numeric(ans))
			expect_equal(length(ans), 1)
			# answer from previous run, might be wrong
			expect_equal( ans, 24.5, tolerance = 1 )
		})

test_that("calculate_WUE_o with wrong column names",{
  expect_error(
    ans <- calculate_WUE_o(EddySample, "GPP_NT_VUT_USTAR50_bla", "VPD_F?_foobar", "TA_F", C = 1.1, Z = 0.2)
